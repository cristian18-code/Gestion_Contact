<?php
    ...
    $ssql = "SELECT laborat.id_registro,
                    t.nombre_tipificacion AS estado,
                    DATE_FORMAT(laborat.fechaRegistro, '%d/%m/%Y') AS fecha_registro,
                    laborat.horaRegistro,
                    laborat.documento,
                    laborat.nombrePaciente,
                    laborat.tipoSolicitud
                    FROM (laboratorios_adomicilio laborat
                    LEFT JOIN tipificaciones t
                    ON laborat.id_tipificacionEstado = t.id_tipificacion)
                    WHERE id_tipificacionEstado != '98' OR id_tipificacionEstado IS NULL";
...
<!-- Scripts -->       
    <title>Laboratorios - Gestion Contact</title>
</head>
<body>
<header>
    <?php 
    ...
    <section>
        ...
        <div>   
        <span class=""><h2>Registros Laboratorios <b>CITAS</b></h2></span>
        ...
        <table id="registros" class="table table-striped table-bordered" style="width:100%">
            ...
            <tbody>
                <?php foreach ($qsqlDatos as $dato) { ?>
                    <tr>
                        <form action="laboratorioCitas_gestor.php" method="post">
