<?php 
    ...
    if ( empty($_POST['user'])) {
    ...
    else if (empty($_POST['estado']) || empty($_POST['observacionBack'])) {                
                    $alert="<script>
                    Swal.fire({
                        icon: 'error',
                        title: 'Oops...',
        ...
        $insertSsql = "UPDATE autorizaciones_fono SET   fechaCierre = STR_TO_DATE('$fechaGestion', '%d/%m/%Y'),
                                                        horaCierre = '$horaGestion',
                                                        observacionesBack = CONCAT(observacionesBack, '$observacionBack //'),
