<?php 
    ...
    if ( empty($_POST['user'])) {
    ...
    else if (empty($_POST['estado']) || empty($_POST['enviar'])) {                
                    $alert="<script>
                    Swal.fire({
                        icon: 'error',
                        title: 'Oops...',
        ...
        $insertSsql = "UPDATE mantenimiento_posventa SET  fecha_Envio = STR_TO_DATE('$fechaGestion', '%d/%m/%Y'),
                                                        hora_Envio = '$horaGestion',
                                                        id_tipificacionEstado = '$estado',
                                                        id_tipificacionenviarA = '$enviar',
