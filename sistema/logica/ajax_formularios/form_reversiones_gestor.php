<?php 
    ...
    if ( empty($_POST['user'])) {
    ...
    else if (empty($_POST['observacionesBack'])) {                
                    $alert="<script>
                    Swal.fire({
                        icon: 'error',
                        title: 'Oops...',
        ...
        $ReversionEfectiva = $_POST['ReversionEfectiva'];
        $observacionesBack = $_POST['observacionesBack'];

        $insertSsql = "UPDATE reversiones_fono SET fechaCierre = STR_TO_DATE('$fechaGestion', '%d/%m/%Y'),
                                                   horaCierre = '$horaGestion',
                                                   observacionesBack = CONCAT(observacionesBack, '$observacionesBack //'),
                                                   reversion_efectiva = '$ReversionEfectiva',
